...
function textNode(text: string): string {
  const clean = text.split(TOC_TAB).join("");
  const preserved = /^\s|\s$/.test(clean);
  return `<w:t${preserved ? ' xml:space="preserve"' : ""}>${escapeXml(clean)}</w:t>`;
}
...
function segmentText(segment: ParagraphBox["segments"][number][number]): string {
  return segment.spans.map((span) => span.text).join("").split(TOC_TAB).join("").trim();
}
